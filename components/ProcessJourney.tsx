  ...
  return (
    <section className="w-full px-6 md:px-12 py-12 md:py-20 bg-white">
      <div className="max-w-[1440px] mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-start">
          {/* Left Column - Step Cards */}
          {/* CHANGE 1: Reduced gap-24 to gap-4 (Make this number higher if you want slightly more space) */}
          <div className="relative flex flex-col gap-4 lg:pl-4">
            {steps.map((step, index) => {
              const isActive = index === activeStep;
              return (
                <motion.div
                ...
                >
                  ...
                  {index !== steps.length - 1 && (
                    <div
                      // CHANGE 2: Changed bottom-[-96px] to bottom-[-16px] to match the new gap-4
                      className="absolute left-[18px] top-[40px] bottom-[-16px] w-[1px] border-l border-dashed z-0 transition-colors duration-500"
                      style={{ borderColor: isActive ? "#215ACD" : "#E2E8F0" }}
                    />
                  )}
                  ...
                  {/* Step Card */}
                  <motion.div
                    // CHANGE 3: Reduced padding to p-3 (was p-4 lg:px-6 lg:py-6)
                    className="flex-1 rounded-[11px] p-3 flex flex-col gap-2 shadow-md w-full relative border"
                    animate={{
                      backgroundColor: isActive ? "#215ACD" : "#FFFFFF",
                      borderColor: isActive ? "#215ACD" : "#F1F5FF",
                      scale: isActive ? 1.02 : 1,
                  ...
                  >
                    <div className="flex justify-between items-center w-full">
                      <motion.h3
                        // Optional: Reduced text size slightly for smaller box feel
                        className="text-[18px] font-semibold font-['Inter'] leading-tight"
                        animate={{ color: isActive ? "#FFFFFF" : "#1E1E1E" }}
                      >
                        {step.title}
                      </motion.h3>
                      <motion.div
                        className="rounded-[4px] px-[8px] py-[4px] flex items-center justify-center"
                        animate={{ backgroundColor: isActive ? "#FFFFFF" : "#F1F5FF" }}
                      >
                        <motion.span
                          className="text-[12px] font-normal font-['Inter']"
                          animate={{ color: isActive ? "#215ACD" : "#64748B" }}
                        >
                          {step.duration}
                        </motion.span>
                    ...
                    <motion.p
                      className="text-[14px] font-normal font-['Inter'] leading-relaxed"
                      animate={{ color: isActive ? "rgba(255,255,255,0.9)" : "#64748B" }}
                    >
                      {step.description}
                    </motion.p>
